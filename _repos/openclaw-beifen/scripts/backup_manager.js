...
async function githubBackup() {
  ...
  // 删除备份仓库里多余文件（保守策略：只清理 workspace 对应内容，不动 .git）
  ...
  run(`git -C ${shellEscape(GITHUB_REPO_DIR)} add -A`, { stdio: 'inherit' });

  const status = run(`git -C ${shellEscape(GITHUB_REPO_DIR)} status --short`).trim();
  if (!status) {
  ...
  run(`git -C ${shellEscape(GITHUB_REPO_DIR)} commit -m ${shellEscape(commitMsg)}`, { stdio: 'inherit' });
  run(`git -C ${shellEscape(GITHUB_REPO_DIR)} push origin ${branch}`, { stdio: 'inherit' });
